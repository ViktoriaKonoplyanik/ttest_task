...
def can_jump(nums):
    ...
    print(
        "Твоя задача — добраться до конца списка. С каждой позиции ты можешь прыгать на количество позиций, не превышающее значение в текущем элементе списка nums[i].\n")
    time.sleep(1)
    ...
    for i, jump in enumerate(nums):
        ...
        if i > max_reach:
            ...
            return False  # Не сможем продолжить.
...
def start_game():
    """
    Функция запускает игру, предлагая пользователю выбрать способ создания списка: случайный, ввод с клавиатуры или выбор из примеров.
